...
```bash
node epub2markup.js path/to/book.epub [output-file] [--tidy-titles]
```
...
- The optional `--tidy-titles` flag (split mode only) cleans up messy chapter headings by preferring the first real title in each file, dropping stray id lines (e.g., `ch01`), and removing duplicate headings before writing files.
